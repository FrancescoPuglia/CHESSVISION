...
export class EnterpriseParser {
  ...
  private static parseRobust(pgnText: string, index: number): PgnGame | null {
    try {
      ...
      const parseResult = pgnParse(pgnText, { startRule: "games" });
      const parseTree = Array.isArray(parseResult)
        ? parseResult
        : [parseResult];
   ...
   */
  private static extractMovesRecursive(
    moves: any[],
    output: PgnMove[],
    moveNumber = 1,
  ): void {
        ...
        move.variations.forEach((variation: any, index: number) => {
          ...
          variationMoves.forEach((vm) => {
            vm.variation = {
              ...
              parentMove:
                output.length > 0 ? output[output.length - 1].san : undefined,
            };
          });
  ...
  private static fallbackParser(pgnText: string): PgnGame | null {
    ...
    const moves: PgnMove[] = [];
    const movePattern =
      /\d+\.\s*([NBRQK]?[a-h]?[1-8]?x?[a-h][1-8](?:=[NBRQ])?[+#]?)/g;
  ...
  private static hasVariations(moves: any[]): boolean {
    ...
    return moves.some(
      (move) =>
        ...
        (move.variations &&
          move.variations.some((v: any) => this.hasVariations(v.moves))),
    );
  }
  ...
  static validate(pgnText: string): {
    ...
    try {
      ...
      if (collection.studies.some((s) => s.fallback)) {
        warnings.push(
          "Some studies parsed with fallback parser - quality may be reduced",
        );
      }
      ...
      errors.push(
        `Critical parse error: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
