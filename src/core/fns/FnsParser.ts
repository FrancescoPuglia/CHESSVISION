...
import { TacticalProblem, TacticalCollection, ProblemDifficulty, TacticalTheme } from '@core/chess/types';
...
export class FnsParser {
   ...
   */
  static parseFnsFile(fnsContent: string, fileName: string = 'Unknown'): TacticalCollection {
    ...
    for (const line of lines) {
      ...
      try {
        const problem = this.parseFnsLine(trimmedLine, `${fileName}_${lineNumber}`);
        if (problem) {
   ...
   */
  private static parseFnsLine(line: string, id: string): TacticalProblem | null {
   ...
   */
  private static analyzeDescription(description: string): { theme?: TacticalTheme; difficulty?: ProblemDifficulty } {
   ...
   */
  static validateSolutionMove(userMove: string, problem: TacticalProblem, moveIndex: number = 0): boolean {
   ...
   */
  static getHint(problem: TacticalProblem, moveIndex: number = 0): string | null {
    if (moveIndex >= problem.solution.length) {
      return null;
    }
