...
// Stockfish types (dynamic import)
...
export class ChessValidator {
  ...
  >();
  private evaluationTimeout = new Map<string, ReturnType<typeof setTimeout>>();
  private chess: Chess;
  ...
  private async initializeEngine(): Promise<void> {
    try {
      // Dynamic import to avoid blocking main thread - fallback if module not available
      try {
        const stockfishModule = await import("stockfish");
        const StockfishEngine = stockfishModule.default || stockfishModule;
        this.engine = new StockfishEngine();
      } catch (importError) {
        console.warn(
          "Stockfish module not available, using fallback validation",
        );
  ...
  private handleEngineInfo(message: string): void {
    ...
    const evaluationId = this.getActiveEvaluationId();
    if (evaluationId && depth >= this.config.depth - 2) {
      // Near final depth
      this.updatePendingEvaluation(evaluationId, {
        evaluation,
        depth,
        bestMoves: pv.slice(0, 3),
      });
    }
  }
  ...
  private updatePendingEvaluation(
    id: string,
  ...
  async validateFlashcard(
    ...
    try {
      ...
      const isSolution =
        problem.solution.includes(userMove) ||
      ...
      return {
        ...
        explanation: this.generateAnalysisExplanation(
          engineResult,
          problem,
          moveObj,
        ),
  ...
  private fallbackValidation(fen: string, move: string): ValidationResult {
    try {
      ...
      return {
        ...
        evaluation,
        bestMoves: [move],
        explanation: this.generateBasicExplanation(
          moveObj,
          isCheck,
          isCheckmate,
          isCapture,
  ...
  private calculatePositionalValue(problem: FlashcardProblem): number {
    switch (problem.type) {
      case "tactical":
        return 2 + problem.difficulty * 0.3;
      case "endgame":
        return 3 + problem.difficulty * 0.2;
      case "positional":
        return 1 + problem.difficulty * 0.4;
      case "opening":
        return 0.5 + problem.difficulty * 0.1;
      default:
        return 1;
    }
  }

  private generateSolutionExplanation(
    problem: FlashcardProblem,
    move: any,
  ): string {
    const themes = {
    ...
    if (move.flags.includes("c"))
      return "Excellent! This capture improves your position.";
    if (move.flags.includes("+"))
      return "Great! This check puts pressure on the opponent.";
...
export function createChessValidator(
