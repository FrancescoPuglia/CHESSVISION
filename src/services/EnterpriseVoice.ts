...
import { encryptData, generateEncryptionKey } from "../utils/security";
...
export class EnterpriseVoice {
  ...
  private setupRecognition(): void {
    ...
    // Note: maxAlternatives is not standardized across browsers
  ...
  private tryAlternativeResults(result: SpeechRecognitionResult): void {
    ...
    for (let i = 0; i < result.length && i < this.config.maxAlternatives; i++) {
      ...
      if (alternative.confidence >= this.config.confidenceThreshold * 0.8) {
        this.processCommand(
          alternative.transcript.trim().toLowerCase(),
  ...
  private parseStandardNotation(
    ...
    const squarePattern =
      /([a-h]|alpha|bravo|charlie|delta|echo|foxtrot|golf|hotel)\s*(1|2|3|4|5|6|7|8|uno|due|tre|quattro|cinque|sei|sette|otto)/gi;
  ...
  private parseNaturalLanguage(
    ...
    if (grammar.specialMoves.castling.some((word) => command.includes(word))) {
      if (
        command.includes("king") ||
        command.includes("corto") ||
        command.includes("short")
      ) {
        return { from: "e1", to: "g1", special: "castling" };
      } else if (
        command.includes("queen") ||
        command.includes("lungo") ||
  ...
  private extractSquares(command: string, grammar: VoiceGrammar): string[] {
    ...
    for (let i = 0; i < words.length - 1; i++) {
      const square = this.normalizeSquare(
        `${words[i]} ${words[i + 1]}`,
        grammar,
      );
      if (square) {
...
// Speech Recognition types are already declared in lib.dom.d.ts
