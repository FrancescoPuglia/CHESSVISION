...
export class SecureStorage {
  ...
  private async storeEncryptionKey(keyId: string, key: CryptoKey): Promise<void> {
    try {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains('storage')) {
          ...
          store.createIndex('encrypted', 'metadata.encrypted', { unique: false });
        }
        
        if (!db.objectStoreNames.contains('sessions')) {
          const sessionStore = db.createObjectStore('sessions', { keyPath: 'id' });
          sessionStore.createIndex('created', 'metadata.created', { unique: false });
        }
  ...
  async getStats(): Promise<StorageStats> {
    ...
    const allItems = await this.performTransaction('storage', 'readonly', (store) => {
      return store.getAll();
    }) as StorageItem[];
  ...
  async cleanup(): Promise<{ itemsRemoved: number; bytesFreed: number }> {
    ...
    console.log(`Cleanup completed: ${itemsRemoved} items removed, ${bytesFreed} bytes freed`);
  ...
  async exportData(): Promise<{ [key: string]: any }> {
    ...
    for (const key of keys) {
      if (!key.startsWith('__system_')) { // Skip system keys
   ...
   */
  async importData(data: { [key: string]: any }, overwrite = false): Promise<void> {
...
// Factory function
export function createSecureStorage(config?: Partial<StorageConfig>): SecureStorage {
...
export class ChessVisionStorage extends SecureStorage {
  ...
  async saveStudy(id: string, study: any): Promise<void> {
    return this.set(`study_${id}`, study, { sensitive: false, expires: this.config.maxAge });
  }
