...
export class ProfessionalStockfish {
  ...
  async analyzePosition(fen: string, levelKey: string): Promise<EngineMove> {
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log(
        `⚡ Analyzing at depth ${config.depth} for ${config.timeLimit}ms...`,
      );
    });
  }
  ...
  private selectProfessionalMove(chess: Chess, moves: any[]): any {
    ...
    const safeMoves = moves.filter((move) => {
      ...
      const isSafe = !responses.some(
        (resp) => resp.captured && resp.to === move.to,
      );
  ...
  private selectAdvancedMove(_chess: Chess, moves: any[]): any {
    ...
    const goodMoves = moves.filter(
      (move) =>
        move.san.includes("e4") ||
        move.san.includes("d4") ||
        move.san.includes("Nf3") ||
        move.san.includes("Bc4") ||
