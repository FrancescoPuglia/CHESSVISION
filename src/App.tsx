...
function App() {
  ...
  const [currentCollection, setCurrentCollection] =
    useState<PgnCollection | null>(null);
  ...
  const speakPieceList = async () => {
    ...
    if (whitePieces.length > 0) {
      text += `${t("white")}: `;
      text += whitePieces
        .map((p) => `${t(p.piece as keyof typeof translations.en)} ${p.square}`)
        .join(", ");
    }
  ...
  const isChessMove = (text: string): boolean => {
    ...
    const movePattern =
      /^[a-h][1-8]([a-h][1-8])?$|^[NBRQK][a-h]?[1-8]?x?[a-h][1-8]$|^O-O(-O)?$/i;
  ...
  const loadStudyPosition = (study: any, moveIndex: number) => {
    try {
      ...
      const success = gameActions.loadStudyPosition(
        moves,
        moveIndex,
        startingFen,
      );
      ...
      if (success) {
        ...
        if (isVoiceEnabled && speechService.current) {
          ...
          speechService.current.speak(
            `Caricata ${position} di ${PgnParser.getStudyTitle(study)}`,
          );
        }
      }
  ...
  const navigateStudyMoves = (direction: number) => {
    ...
    const newMoveIndex = Math.max(
      0,
      Math.min(maxMoves, studyMoveIndex + direction),
    );
  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      if (file.name.endsWith(".fns")) {
        ...
        throw new Error(
          "Unsupported file format. Please use .pgn or .fns files.",
        );
      }
    } catch (error) {
  ...
  return (
    ...
    >
          ...
          <div style={{ marginBottom: "1.5rem" }}>
            ...
            <div style={{ marginBottom: "1rem" }}>
              ...
              <select
                ...
                onChange={(e) =>
                  setStudyTimeLimit(parseInt(e.target.value) * 60)
                }
                style={{
                  width: "100%",
                  padding: "0.5rem",
          ...
          <div style={{ marginBottom: "1.5rem" }}>
            ...
            >
              <input
                ...
                style={{ display: "none" }}
              />
              {selectedFile ? `✓ ${selectedFile.name}` : t("uploadOrDrop")}
            </label>
            ...
            <button
              onClick={() => {
                ...
                const collection = FnsParser.parseFnsFile(
                  sampleFns,
                  "Sample Tactics",
                );
                tacticalActions.loadCollection(collection);
            ...
            {tacticalState.currentCollection && (
              ...
              >
                ...
                {tacticalState.currentCollection.problems.length} problems)
              </button>
            )}
              ...
              <>
                ...
                <button
                  onClick={() => {
                    ...
                    const currentStudy =
                      currentCollection.studies[currentStudyIndex];
                    const testMoveIndex = Math.min(
                      3,
                      currentStudy.moves.length,
                    );
              ...
              >
                  ...
                  {currentCollection.studies.map((study, index) => (
                    <div
                      ...
                      onClick={() => {
                        ...
                        if (isVoiceEnabled && speechService.current) {
                          speechService.current.speak(
                            `Selezionato studio ${index + 1}: ${PgnParser.getStudyTitle(study)}`,
                          );
                        }
                      }}
                      style={{
                        backgroundColor:
                          index === currentStudyIndex ? "#8b5cf6" : "#3d4251",
                        padding: "1rem",
                        borderRadius: "6px",
                        cursor: "pointer",
                        border:
                    ...
                    >
                      ...
                      >
                        <h4
                          style={{
                            color:
                              index === currentStudyIndex ? "#fff" : "#ffd700",
                            margin: 0,
                            fontSize: "1rem",
                            fontWeight: "bold",
                          }}
                        >
                          {index + 1}.{" "}
                          {study.headers.Event || "Studio Senza Nome"}
                        </h4>
                      ...
                      <div
                        style={{
                          fontSize: "0.8rem",
                          color:
                            index === currentStudyIndex ? "#e0e0e0" : "#a0a0a0",
                          marginBottom: "0.5rem",
                        }}
                      >
                      ...
                      <div
                        style={{
                          ...
                          fontSize: "0.8rem",
                          color:
                            index === currentStudyIndex ? "#d0d0d0" : "#888",
                        }}
                      >
                ...
                >
                  ...
                  >
                    <span style={{ color: "#a0a0a0", fontSize: "0.9rem" }}>
                      ...
                      <strong style={{ color: "#ffd700" }}>
                        {currentStudyIndex + 1} di{" "}
                        {currentCollection.studies.length}
                      </strong>
                    </span>
                    <span style={{ color: "#a0a0a0", fontSize: "0.9rem" }}>
                      Mossa:{" "}
                      <strong style={{ color: "#8b5cf6" }}>
                        {studyMoveIndex} di{" "}
                        {
                          currentCollection.studies[currentStudyIndex].moves
                            .length
                        }
                      </strong>
                    </span>
                  ...
                  <div style={{ fontSize: "0.8rem", color: "#666" }}>
                    💡 Clicca su uno studio per caricarlo • Usa PageUp/PageDown
                    per navigare • ← → per le mosse
                  </div>
                </div>
              </div>
            )}
            ...
            >
              ...
              >
                ...
                <div style={{ color: "#a0a0a0", fontSize: "0.9rem" }}>
                  {studyMoveIndex + 1} /{" "}
                  {currentCollection.studies[currentStudyIndex].moves.length}
                </div>
              </div>
                  ...
                  >
                    <button
                      ...
                      style={{
                        padding: "0.5rem 1rem",
                        backgroundColor:
                          currentStudyIndex <= 0 ? "#666" : "#8b5cf6",
                        color: "white",
                        border: "none",
                        borderRadius: "6px",
                        cursor:
                          currentStudyIndex <= 0 ? "not-allowed" : "pointer",
                        fontSize: "0.8rem",
                      }}
                    >
                      ⬆️ Studio Precedente
            ...
            <InteractiveChessBoard
              ...
              lastMove={
                gameState.moves.length > 0
                  ? {
                      from:
                        gameState.moves[gameState.moves.length - 1]?.from ||
                        "a1",
                      to:
                        gameState.moves[gameState.moves.length - 1]?.to || "a1",
                    }
                  : undefined
              }
            />
          ...
          >
            ...
            <div style={{ display: "grid", gap: "0.75rem" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                ...
                <span
                  style={{
                    color: isBoardVisible ? "#10b981" : "#ef4444",
                    fontWeight: "bold",
                  }}
                >
              ...
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                ...
                <span
                  style={{
                    color: isVoiceEnabled ? "#10b981" : "#666",
                    fontWeight: "bold",
                  }}
                >
            ...
            <div style={{ display: "grid", gap: "0.75rem" }}>
              <button
                onClick={() => {
                  if (isVoiceEnabled && speechService.current) {
                    speechService.current.speak(
                      "Welcome to ChessVision blindfold training. Use voice commands or keyboard shortcuts to navigate.",
                    );
                  }
                }}
                style={{
      ...
      >
        <p>CHESSVISION v2.0 - Blindfold Chess Training Platform</p>
        <p>
          🎯 Focus: Voice Commands • Timer • PGN Studies • Engine Games •
          Bilingue EN/IT
        </p>
      </div>
      ...
      {showStudyMode &&
        currentCollection &&
        currentCollection.studies.length > 0 && (
          <StudyMode
      ...
      {showTacticalMode && tacticalState.currentProblem && (
        <TacticalMode
          ...
          onNext={() => {
            ...
            if (
              tacticalState.currentProblemIndex >=
              (tacticalState.currentCollection?.problems.length || 0) - 1
            ) {
              setShowTacticalMode(false);
              if (isVoiceEnabled && speechService.current) {
                speechService.current.speak(
                  "Allenamento tattico completato! Ottimo lavoro!",
                );
              }
            }
          }}
