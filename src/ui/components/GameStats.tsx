  ...
  return (
    <div className={`game-stats ${className}`}>
      ...
      <StatCard
        title="Precisione"
        value={`${precision}%`}
        subtitle={precision >= 80 ? "Eccellente!" : precision >= 60 ? "Buono" : "Puoi migliorare"}
        color={precision >= 80 ? '#10b981' : precision >= 60 ? '#ffd700' : '#ef4444'}
      />

      <StatCard
        title="Studi Completati"
        value={completed}
        subtitle={completed > 0 ? `${completed} ${completed === 1 ? 'studio' : 'studi'}` : undefined}
      />

      <StatCard
        title="Serie Attuale"
        value={series}
        subtitle={series > 5 ? "In fiamme! 🔥" : series > 0 ? "Continua così!" : undefined}
