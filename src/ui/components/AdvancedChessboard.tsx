...
export const AdvancedChessboard: React.FC<AdvancedChessboardProps> = ({
  ...
  showLastMove: _showLastMove = true,
  showPossibleMoves: _showPossibleMoves = true,
  premovesEnabled: _premovesEnabled = false,
  onMove,
  onPremove: _onPremove,
  animations: _animations = true,
  ...
  const [_premove, _setPremove] = useState<ChessMove | null>(null);

  // Custom DOM diff algorithm inspired by Chessground
  const updateBoardDOM = useCallback((newBoardState: ChessSquare[][]) => {
      ...
      row.forEach((square, fileIndex) => {
        ...
        if (square.piece) {
          if (!pieceElement) {
            ...
            const currentClass = pieceElement.className.split(' ').find(cls => cls.match(/^[wb][PRNBQK]$/));
            if (currentClass !== currentPiece) {
              pieceElement.className = `chess-piece ${square.piece.color} ${square.piece.type}`;
              (pieceElement as HTMLElement).style.backgroundImage = `url(/pieces/${customPieceSet}/${currentPiece}.svg)`;
            }
          }
        } else {
          if (pieceElement) {
        ...
        const classesToRemove = ['selected', 'highlighted', 'last-move', 'possible-move'];
  ...
  const gameToSquares = useCallback((): ChessSquare[][] => {
    ...
    for (let rankIndex = 0; rankIndex < 8; rankIndex++) {
      ...
      for (let fileIndex = 0; fileIndex < 8; fileIndex++) {
        ...
        const square: ChessSquare = {
          ...
          isLastMove: lastMove ? (lastMove.from === squareKey || lastMove.to === squareKey) : false,
    ...
    return orientation === 'black' ? squares.slice().reverse().map(rank => rank.slice().reverse()) : squares;
  ...
  const updateSVGOverlay = useCallback(() => {
    ...
    annotations.forEach(annotation => {
      if (annotation.type === 'arrow' && annotation.from && annotation.to) {
        const arrow = createArrowElement(annotation.from, annotation.to, annotation.color, annotation.opacity);
        svg.appendChild(arrow);
      } else if (annotation.type === 'circle' && annotation.square) {
        const circle = createCircleElement(annotation.square, annotation.color, annotation.opacity);
  ...
  const squareToCoordinates = (square: string): {x: number, y: number} => {
    ...
    const x = (orientation === 'white' ? file : 7 - file) * squareSize + squareSize / 2;
  ...
  const handleSquareClick = useCallback((square: string) => {
    ...
    if (selectedSquare) {
      if (selectedSquare === square) {
        ...
        const relevantMoves = moveStrings.filter(move => move.startsWith(square));
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent, square: string) => {
    ...
    const squareData = boardState.find(row => 
      row.find(sq => `${sq.file}${sq.rank}` === square)
