...
import { TacticalProblem, TacticalAttempt, TacticalConfig, ProblemState } from '@core/chess/types';
...
interface TacticalModeProps {
  ...
  onComplete: (solved: boolean, attempts: TacticalAttempt[], timeMs: number) => void;
  onExit: () => void;
  onNext: () => void;
...
export const TacticalMode: React.FC<TacticalModeProps> = ({
  ...
  // Timer countdown
  useEffect(() => {
    if (state.timeRemaining && state.timeRemaining > 0 && state.status === 'thinking') {
  ...
  const handleMoveSubmit = (moveInput: string) => {
    ...
    const isCorrect = FnsParser.validateSolutionMove(userMove, problem, state.currentMoveIndex);
  ...
  const handleSkip = () => {
    ...
    setState(prev => ({
      ...prev,
      status: 'failed',
      message: 'Problema saltato. La soluzione era: ' + problem.solution.join(' '),
      messageType: 'warning'
    }));
  ...
  const canUseHints = state.hintsUsed < config.maxHints && state.status === 'thinking';
  const progressPercent = ((state.currentMoveIndex) / problem.solution.length) * 100;
      ...
      <div style={{
        ...
        <div style={{
          ...
          border: `2px solid ${
            ...
            state.messageType === 'error' ? '#ef4444' :
            state.messageType === 'warning' ? '#f59e0b' : '#8b5cf6'
          }`
        }}>
          <p style={{
            color: state.messageType === 'success' ? '#10b981' :
                   state.messageType === 'error' ? '#ef4444' :
                   state.messageType === 'warning' ? '#f59e0b' : '#ffd700',
            fontWeight: 'bold',
            margin: 0,
            fontSize: '1.1rem',
            textAlign: 'center'
        ...
        <div style={{
          ...
          <div><strong>Difficoltà:</strong> {problem.difficulty ? `${problem.difficulty}/5` : 'N/D'}</div>
          <div><strong>Tentativi:</strong> {state.attempts.length}</div>
          <div><strong>Suggerimenti:</strong> {state.hintsUsed}/{config.maxHints}</div>
          {problem.source && <div><strong>Fonte:</strong> {problem.source}</div>}
        </div>
      </div>
    </div>
  );
