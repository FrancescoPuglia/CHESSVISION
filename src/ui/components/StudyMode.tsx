  ...
  const handleMoveSubmit = () => {
    ...
    if (state.expectedMove && userMove === state.expectedMove) {
      ...
      if (moveResult) {
        ...
        if (isStudyComplete) {
          // Study completed!
          const finalScore = Math.round((state.score + 100) * (state.timeRemaining / timeLimit));
          setState(prev => ({
            ...prev,
            studyGame: newGame,
          ...
          setState(prev => ({
            ...
            messageType: 'success',
            score: prev.score + Math.round(50 * (prev.timeRemaining / timeLimit)),
            userInput: ''
          }));
        }
      }
    ...
    <div style={{
      ...
      <div style={{
          ...
          <div style={{ marginBottom: '2rem' }}>
            <div style={{
              ...
              <button
                ...
                style={{
                  padding: '1rem 2rem',
                  backgroundColor: state.userInput.trim() && state.isTimerActive ? '#10b981' : '#666',
                  color: 'white',
                  border: 'none',
                  borderRadius: '8px',
                  cursor: state.userInput.trim() && state.isTimerActive ? 'pointer' : 'not-allowed',
                  fontSize: '1rem',
                  fontWeight: 'bold'
                }}
              >
        ...
        <div style={{
          ...
          <div><strong>Tentativi:</strong> {state.attempts}</div>
          <div><strong>Precisione:</strong> {state.attempts > 0 ? Math.round((state.currentMoveIndex / state.attempts) * 100) : 100}%</div>
        </div>
      </div>
    </div>
  );
