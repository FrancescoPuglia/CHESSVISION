...
export default function EngineTestSuite() {
  ...
  const [summary, setSummary] = useState<{[key: string]: {passed: number, total: number}}>({});
  ...
  const runFullTestSuite = async () => {
    ...
    for (const level of levels) {
      ...
      for (const position of testPositions) {
        ...
        try {
          const result = await professionalStockfish.analyzePosition(position.fen, level.key);
          ...
          console.error(`Test failed for ${level.key} on ${position.name}:`, error);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-300 text-lg">
            Validazione sistematica dei livelli di forza secondo l'analisi strategica
          </p>
        </div>
        ...
        <div className="bg-gray-800 rounded-xl p-6 mb-8 border border-gray-700">
          <div className="flex justify-between items-center mb-4">
            <div>
              ...
              <p className={`text-lg ${engineReady ? 'text-green-400' : 'text-yellow-400'}`}>
          ...
          {Object.keys(summary).length > 0 && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
              {Object.entries(summary).map(([levelKey, stats]) => {
                const level = professionalStockfish.getAllLevels().find(l => l.key === levelKey);
                const percentage = stats.total > 0 ? (stats.passed / stats.total * 100).toFixed(1) : '0';
                
                return (
                  <div key={levelKey} className="bg-gray-700 rounded-lg p-3 text-center">
                    <h3 className={`font-semibold text-sm ${getLevelColor(level?.elo || 0)}`}>
                      {level?.name || levelKey}
                    </h3>
                    <div className="mt-2">
        ...
        {testResults.length > 0 && (
          <div className="space-y-6">
            <h2 className="text-2xl font-bold mb-4">📊 Risultati Dettagliati</h2>
                ...
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="text-xl font-semibold flex items-center gap-3">
                      ...
                      <span className={getLevelColor(result.elo)}>{result.level}</span>
                      <span className="text-gray-300">→</span>
                      <span>{result.position}</span>
                    </h3>
                    <p className="text-sm text-gray-400 mt-1">
                      ELO: {result.elo} | Profondità: {result.depth} | Tempo: {result.time}ms
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-gray-700 rounded-lg p-4">
                    ...
                    <div className="mt-2 space-y-1 text-sm">
                      <p className={result.isLegal ? 'text-green-400' : 'text-red-400'}>
                        {result.isLegal ? '✓ Legale' : '✗ Illegale'}
                      </p>
                      <p className={result.isExpected ? 'text-green-400' : 'text-yellow-400'}>
        ...
        <div className="mt-12 bg-gradient-to-r from-blue-600/20 to-purple-600/20 rounded-xl p-6 border border-blue-500/30">
          <h2 className="text-xl font-bold mb-4">💡 Informazioni sulla Suite di Test</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <h3 className="font-semibold mb-2 text-blue-400">Cosa Testiamo</h3>
            ...
            <div>
              <h3 className="font-semibold mb-2 text-purple-400">Livelli di Test</h3>
              <ul className="text-sm space-y-1 text-gray-300">
                <li>• <span className="text-gray-400">800-1200:</span> Principianti</li>
                <li>• <span className="text-green-400">1200-1800:</span> Intermedi</li>
                <li>• <span className="text-blue-400">1800-2200:</span> Avanzati</li>
                <li>• <span className="text-purple-400">2200+:</span> Professionali</li>
              </ul>
            </div>
          </div>
        </div>
