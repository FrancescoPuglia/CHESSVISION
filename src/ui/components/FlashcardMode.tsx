...
export const FlashcardMode: React.FC<FlashcardModeProps> = ({
    ...
    const generateFlashcards = () => {
      ...
      studies.forEach((study, studyIndex) => {
        study.moves.forEach((move, moveIndex) => {
          if (moveIndex > 0) { // Skip first move, use as setup
  ...
  const handleMoveSubmit = () => {
    ...
    setState(prev => ({
      ...
      sessionStats: {
        ...
        correct: isCorrect ? prev.sessionStats.correct + 1 : prev.sessionStats.correct,
        streak: isCorrect ? prev.sessionStats.streak + 1 : 0
      }
    }));
  };

  const handlePerformanceRating = (rating: 'again' | 'hard' | 'good' | 'easy') => {
    if (!state.currentCard) return;

    // Anki-style spaced repetition algorithm
    const updateCard = (card: FlashcardData, performance: string): FlashcardData => {
      ...
      switch (performance) {
        ...
        case 'hard':
          ...
          break;
        case 'good':
          newInterval = card.reviewCount === 0 ? 1 : 
                        card.reviewCount === 1 ? 6 : 
                        Math.ceil(card.interval * card.easeFactor);
          break;
        case 'easy':
          newInterval = card.reviewCount === 0 ? 4 :
                        card.reviewCount === 1 ? 10 :
  ...
  const progress = state.totalCards > 0 ? ((state.cardIndex + 1) / state.totalCards) * 100 : 0;
  const accuracy = state.sessionStats.reviewed > 0 ? 
    Math.round((state.sessionStats.correct / state.sessionStats.reviewed) * 100) : 100;
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ color: '#a0a0a0', fontSize: '0.9rem' }}>Progresso</div>
          </div>
          ...
          <div style={{
            ...
            <div style={{ color: '#a0a0a0', fontSize: '0.9rem' }}>Precisione</div>
          </div>
        ...
        <div style={{
          ...
          marginBottom: '2rem',
          border: `2px solid ${
            state.messageType === 'success' ? '#10b981' :
            state.messageType === 'error' ? '#ef4444' : '#8b5cf6'
          }`
        }}>
          <h3 style={{
            color: state.messageType === 'success' ? '#10b981' :
                   state.messageType === 'error' ? '#ef4444' : '#ffd700',
            margin: 0,
            fontSize: '1.2rem',
            textAlign: 'center'
          }}>
          ...
          <div style={{ flex: '0 0 300px' }}>
              ...
              <div style={{ marginBottom: '2rem' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <input
                    ...
                    onChange={(e) => setState(prev => ({ ...prev, userMove: e.target.value }))}
                  ...
                  <button
                    ...
                    style={{
                      ...
                      backgroundColor: state.userMove.trim() ? '#8b5cf6' : '#666',
                      color: 'white',
                      border: 'none',
                      borderRadius: '6px',
            ...
            {state.currentCard && (
              <div style={{
                ...
                <div><strong>Carta:</strong> {state.currentCard.id}</div>
                <div><strong>Revisioni:</strong> {state.currentCard.reviewCount}</div>
                <div><strong>Intervallo:</strong> {state.currentCard.interval} giorni</div>
                <div><strong>Facilità:</strong> {state.currentCard.easeFactor.toFixed(1)}</div>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{
          ...
          <ul style={{ margin: '0.5rem 0', paddingLeft: '1rem' }}>
            ...
            <li>Inserisci la mossa in notazione algebrica (es. Nf3, e4, O-O)</li>
            <li>Dopo la risposta, valuta la difficoltà per l'algoritmo Anki</li>
