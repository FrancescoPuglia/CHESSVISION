...
import { BlindModeService, BlindModeSettings, BlindModeCommand } from '@services/accessibility/BlindModeService';
...
export const BlindModePanel: React.FC<BlindModePanelProps> = ({
  ...
  const [settings, setSettings] = useState<BlindModeSettings>(blindModeService.getSettings());
  const [commandInput, setCommandInput] = useState('');
  const [commandHistory, setCommandHistory] = useState<string[]>([]);
  const [isBlindModeActive, setIsBlindModeActive] = useState(blindModeService.isBlindModeEnabled());
  const [availableCommands] = useState<BlindModeCommand[]>(blindModeService.getAvailableCommands());
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h2 id="blind-mode-title" style={{ color: '#ffd700', margin: 0, fontSize: '2rem' }}>
        ...
        <div style={{ marginBottom: '2rem' }}>
          ...
          >
            {isBlindModeActive ? '✅ Modalità Accessibilità ATTIVA' : '❌ Modalità Accessibilità DISATTIVA'}
          </button>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
          {/* Settings Panel */}
          <div>
            <h3 style={{ color: '#8b5cf6', marginBottom: '1.5rem' }}>⚙️ Impostazioni</h3>
            ...
            <div style={{ display: 'grid', gap: '1rem' }}>
              {[
                { key: 'announceOpponentMoves', label: 'Annuncia mosse avversario' },
                ...
                { key: 'keyboardNavigationEnabled', label: 'Navigazione tastiera' },
                ...
                >
                  <input
                    type="checkbox"
                    checked={settings[key as keyof BlindModeSettings] as boolean}
                    onChange={(e) => handleSettingChange(key as keyof BlindModeSettings, e.target.checked)}
                    style={{
                      marginRight: '0.5rem',
                      transform: 'scale(1.2)',
                    }}
          ...
          {/* Command Interface */}
          <div>
            <h3 style={{ color: '#8b5cf6', marginBottom: '1.5rem' }}>🎤 Interfaccia Comandi</h3>
            
            {/* Command Input */}
            <form onSubmit={handleCommandSubmit} style={{ marginBottom: '1.5rem' }}>
            ...
            {/* Quick Commands */}
            <div style={{ marginBottom: '1.5rem' }}>
              <h4 style={{ color: '#a0a0a0', marginBottom: '1rem', fontSize: '0.9rem' }}>
                Comandi Rapidi:
              </h4>
        ...
        <div style={{ marginTop: '2rem', borderTop: '2px solid #333', paddingTop: '2rem' }}>
          <h3 style={{ color: '#8b5cf6', marginBottom: '1.5rem' }}>📖 Riferimento Comandi</h3>
        ...
        >
          ...
          <ul style={{ color: '#a0a0a0', margin: 0, paddingLeft: '1.5rem' }}>
            <li>Usa <strong>Alt + [comando]</strong> per accesso rapido (es: Alt+L per ultima mossa)</li>
            <li>Il campo comandi supporta sia comandi che mosse scacchistiche (e4, Nf3, etc.)</li>
            <li>Premi <strong>Escape</strong> per sentire l'elemento corrente con focus</li>
            <li>Usa <strong>H</strong>, <strong>F</strong>, <strong>E</strong> per navigare per intestazioni, form ed edit</li>
            <li>Tutti i comandi sono compatibili con screen reader (NVDA, JAWS, VoiceOver)</li>
          </ul>
        </div>
      </div>
    </div>
