...
import { StockfishService, EngineStrength, ENGINE_PRESETS } from '@services/engine/StockfishService';
...
export const EngineGame: React.FC<EngineGameProps> = ({
  ...
  const [engineStrength, setEngineStrength] = useState<EngineStrength>('intermediate');
  ...
  useEffect(() => {
    if (gameStarted && !gameState.isGameOver && engine?.isEngineReady()) {
      ...
      const isEngineTurn = (playerColor === 'white' && currentTurn === 'black') ||
  ...
  }, [gameState.currentMove, gameStarted, playerColor, engine, isEngineThinking]);
  ...
  const startNewGame = () => {
    ...
    if (isVoiceEnabled && speechService) {
      ...
      speechService.speak(`Nuova partita iniziata. Giochi con i pezzi ${colorText} contro ${engineStrength} engine.`);
    }
  ...
  const processVoiceMove = (voiceInput: string): string | null => {
    ...
    const movePattern = /^[a-h][1-8]([a-h][1-8])?$|^[NBRQK][a-h]?[1-8]?x?[a-h][1-8]$/i;
      ...
      <div style={{
        ...
        {!gameStarted && (
          <div style={{ marginBottom: '2rem' }}>
            <h3 style={{ color: '#8b5cf6', marginBottom: '1rem' }}>Impostazioni Partita</h3>
            
            {/* Strength Selection */}
            <div style={{ marginBottom: '1rem' }}>
              <label style={{ color: '#a0a0a0', display: 'block', marginBottom: '0.5rem' }}>
                Livello Motore:
              </label>
              <select
              ...
              >
                {Object.keys(ENGINE_PRESETS).map(strength => (
                  <option key={strength} value={strength}>
                    {engine?.getStrengthInfo(strength as EngineStrength) || strength}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div style={{ marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', gap: '1rem' }}>
                <label style={{ color: '#a0a0a0', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <input
                    type="radio"
                    value="white"
            ...
            >
              {engine?.isEngineReady() ? '🚀 Inizia Partita' : '⏳ Caricamento motore...'}
            </button>
          </div>
        )}
            ...
            <div style={{ marginBottom: '1rem' }}>
              <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.5rem' }}>
                ...
                <button
                  ...
                  disabled={!moveInput.trim() || isEngineThinking || gameState.isGameOver}
                  style={{
              ...
              {isVoiceEnabled && (
                <button
                  ...
                  disabled={isListening || isEngineThinking || gameState.isGameOver}
                  style={{
                    width: '100%',
                    padding: '0.75rem',
