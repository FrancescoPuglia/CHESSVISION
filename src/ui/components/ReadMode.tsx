...
export const ReadMode: React.FC<ReadModeProps> = ({
  ...
  useEffect(() => {
    ...
    if (
      state.isPlaying &&
      !state.isPaused &&
      state.currentMoveIndex < study.moves.length
    ) {
  ...
  const togglePlay = () => {
    setState((prev) => ({
      ...
      isPaused: false,
      currentlyReading: prev.isPlaying
        ? "Lettura in pausa"
        : "Ripresa lettura automatica",
    }));
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {currentMove && (
          ...
          >
            ...
            {state.readComments && currentMove.comment && (
              ...
              >
                💬 <strong>Commento dell&apos;autore:</strong>{" "}
                {currentMove.comment}
              </div>
            )}
          </div>
        ...
        >
          <div>
            <strong>Autore:</strong> {study.headers.White || "Sconosciuto"}
          </div>
          <div>
            <strong>Mosse totali:</strong> {study.moves.length}
          </div>
          <div>
            <strong>Data:</strong> {study.headers.Date || "N/A"}
          </div>
        </div>
      </div>
    </div>
