...
export const useChessGame = (): [GameState, GameActions] => {
  ...
  const [lastMessage, setLastMessage] = useState(
    "Benvenuto! Inserisci una mossa per iniziare.",
  );
        ...
        if (studyMoves.length > 0 && studyIndex < studyMoves.length) {
          ...
          if (result) {
            if (result.san === expectedMove) {
              ...
              setMessage(
                `Mossa legale ma non quella attesa. Atteso: ${expectedMove}`,
                "error",
              );
  ...
  const loadPgnStudy = useCallback(
    (pgnText: string) => {
      try {
        ...
        setMessage(
          `Studio caricato: "${studyName}" (${movesList.length} mosse)`,
          "success",
        );
        ...
        setMessage(
          "Errore nel caricamento del PGN. Verifica il formato.",
          "error",
        );
  ...
  const loadStudyPosition = useCallback(
    (moves: string[], moveIndex: number, startingFen?: string): boolean => {
      try {
        // Create game from starting FEN if provided, otherwise from standard position
        const newGame = startingFen
          ? new ChessGame(startingFen)
          : new ChessGame();
